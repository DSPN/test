...
{ set +x; } 2>/dev/null; echo -e "\$bar\n### cqlsh - show version\n\$bar\n"; set -x
echo "show version; exit" | cqlsh $ip -u cassandra -p cassandra

cat > getDcName.cqlsh << ENDMSG
select data_center from system.local;
ENDMSG
cqlsh $ip -u cassandra -p cassandra -f getDcName.cqlsh

datacenterName=$(cqlsh $ip -u cassandra -p cassandra -f getDcName.cqlsh | sed -n '4p' | xargs)
...
cqlsh $ip -u cassandra -p cassandra -f test.cqlsh

########## current bug in 4.0.alpha4

cat > select1.cqlsh << ENDMSG
select color from test_keyspace.test_table where id=4;
select color from test_keyspace.test_table where id in(1,4,10);
ENDMSG
cqlsh $ip -u cassandra -p cassandra -f select1.cqlsh
